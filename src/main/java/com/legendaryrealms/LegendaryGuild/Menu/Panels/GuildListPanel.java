...
public class GuildListPanel extends MenuDraw {
    ...
    // 新增：使用默认排序的构造函数
    public GuildListPanel(Player p, int page) {
        this(p, page, LegendaryGuild.getInstance().getFileManager().getGuildListFile().getDefaultSort());
    }
        ...
        if (!dealEssentailsButton(e.getRawSlot())){
            ...
            if (menuItem != null){
                switch (menuItem.getFuction()){
                    ...
                    case "sort": {
                        // 使用配置文件中定义的排序选项
                        List<String> availableSorts = legendaryGuild.getFileManager().getGuildListFile().getAvailableSorts();
                        if (!legendaryGuild.getFileManager().getGuildListFile().isSortEnabled() || availableSorts.isEmpty()) {
                            return;
                        }
                        
                        // 找到当前排序在配置列表中的位置
                        String currentSortName = sort.name();
                        int currentIndex = availableSorts.indexOf(currentSortName);
                        ...
                        if (currentIndex == -1) {
                            ...
                            // 切换到下一个排序方式
                            currentIndex = (currentIndex + 1) % availableSorts.size();
                        }
                        
                        try {
                            Sort nextSort = Sort.valueOf(availableSorts.get(currentIndex));
                            GuildListPanel list = new GuildListPanel(p, 1, nextSort);
                            list.loadGuilds(1);
                            list.open();
                        } catch (IllegalArgumentException e1) {
                            // 如果配置的排序方式无效，使用默认排序
                            Sort nextSort = legendaryGuild.getFileManager().getGuildListFile().getDefaultSort();
                            GuildListPanel list = new GuildListPanel(p, 1, nextSort);
                            list.loadGuilds(1);
                            list.open();
                        }
